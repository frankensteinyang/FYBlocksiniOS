...
typedef NS_ENUM(NSUInteger, FYCalendarOrientation) {
    FYCalendarOrientationLandscape,
    FYCalendarOrientationPortrait
};

@interface FYCalendar (DataSourceAndDelegate)
...
- (FYCalendarCellShape)preferredCellShapeForDate:(NSDate *)date;
...
@interface FYCalendar () <UICollectionViewDataSource, UICollectionViewDelegate> {
...
@property (strong, nonatomic) FYCalendarAnimator         *animator;

@property (weak  , nonatomic) FYCalendarHeader           *header;
@property (weak  , nonatomic) FYCalendarHeaderTouchDeliver *deliver;
...
@property (assign, nonatomic) FYCalendarOrientation      orientation;

@property (readonly, nonatomic) BOOL floatingMode;
@property (readonly, nonatomic) BOOL hasValidateVisibleLayout;
@property (readonly, nonatomic) NSArray *visibleStickyHeaders;
@property (readonly, nonatomic) FYCalendarOrientation currentCalendarOrientation;

@property (readonly, nonatomic) id<FYCalendarDelegateAppearance> delegateAppearance;
...
- (void)reloadDataForCell:(FYCalendarCell *)cell atIndexPath:(NSIndexPath *)indexPath;
...
- (void)initialize {
    ...
    _appearance = [[FYCalendarAppearance alloc] init];
    ...
    _headerHeight     = FYCalendarAutomaticDimension;
    _weekdayHeight    = FYCalendarAutomaticDimension;
    
    _preferredHeaderHeight  = FYCalendarAutomaticDimension;
    _preferredWeekdayHeight = FYCalendarAutomaticDimension;
    _preferredRowHeight     = FYCalendarAutomaticDimension;
    
    _scrollDirection = FYCalendarScrollDirectionHorizontal;
    _scope = FYCalendarScopeMonth;
    ...
    FYCalendarFlowLayout *collectionViewLayout = [[FYCalendarFlowLayout alloc] init];
    collectionViewLayout.calendar = self;
    
    FYCalendarCollectionView *collectionView = [[FYCalendarCollectionView alloc] initWithFrame:CGRectZero
    ...
    [collectionView registerClass:[FYCalendarCell class] forCellWithReuseIdentifier:@"cell"];
    [collectionView registerClass:[FYCalendarStickyHeader class] forSupplementaryViewOfKind:UICollectionElementKindSectionHeader withReuseIdentifier:@"header"];
    ...
    self.animator = [[FYCalendarAnimator alloc] init];
...
- (void)setBounds:(CGRect)bounds
{
    ...
    if (!CGRectIsEmpty(bounds) && self.animator.state == FYCalendarTransitionStateIdle) {
...
- (void)setFrame:(CGRect)frame
{
    ...
    if (!CGRectIsEmpty(frame) && self.animator.state == FYCalendarTransitionStateIdle) {
        [self invalidateViewFrames];
    }
}
...
- (void)layoutSubviews
{
    ...
    if (_needsAdjustingViewFrame) {
        ...
        BOOL needsAdjustingBoundingRect = self.scope == FYCalendarScopeMonth && !_showsPlaceholders && !self.hasValidateVisibleLayout;
        
        _contentView.frame = self.bounds;
        
        if (_needsLayoutForWeekMode) _scope = FYCalendarScopeMonth;
        ...
        CGFloat weekdayWidth = self.contentView.fy_width/_weekdays.count;
        ...
        if (_needsLayoutForWeekMode) _scope = FYCalendarScopeWeek;
        
        _header.frame = CGRectMake(0, 0, self.fy_width, headerHeight);
        ...
        _deliver.frame = CGRectMake(_header.fy_left, _header.fy_top, _header.fy_width, headerHeight+weekdayHeight);
        _deliver.hidden = _header.hidden;
        if (!self.floatingMode) {
            switch (_scope) {
                case FYCalendarScopeMonth: {
                    CGFloat contentHeight = rowHeight*6 + padding*2;
                    _daysContainer.frame = CGRectMake(0, headerHeight+weekdayHeight, self.fy_width, contentHeight);
                    _collectionView.frame = _daysContainer.bounds;
                    if (needsAdjustingBoundingRect) {
                        self.animator.state = FYCalendarTransitionStateInProgress;
                        [self boundingRectWillChange:NO];
                        self.animator.state = FYCalendarTransitionStateIdle;
                    }
                    break;
                }
            ...
            CGFloat contentHeight = _contentView.fy_height;
            _daysContainer.frame = CGRectMake(0, 0, self.fy_width, contentHeight);
            _collectionView.frame = _daysContainer.bounds;
            
        }
        _topBorder.frame = CGRectMake(0, -1, self.fy_width, 1);
        _bottomBorder.frame = CGRectMake(0, self.fy_height, self.fy_width, 1);
    ...
    if (_needsLayoutForWeekMode) {
        ...
        _scope = FYCalendarScopeWeek;
        [self.animator performScopeTransitionFromScope:FYCalendarScopeMonth toScope:FYCalendarScopeWeek animated:NO];
    } else {
...
- (CGSize)sizeThatFits:(CGSize)size
{
    switch (self.animator.transition) {
        case FYCalendarTransitionNone:
            return [self sizeThatFits:size scope:_scope];
        case FYCalendarTransitionWeekToMonth:
            if (self.animator.state == FYCalendarTransitionStateInProgress) {
                return [self sizeThatFits:size scope:FYCalendarScopeMonth];
            }
        case FYCalendarTransitionMonthToWeek:
            break;
    }
    return [self sizeThatFits:size scope:FYCalendarScopeWeek];
}
...
- (CGSize)sizeThatFits:(CGSize)size scope:(FYCalendarScope)scope
{
    ...
    if (!self.floatingMode) {
        ...
        return CGSizeMake(size.width, self.fy_height);
    }
    return size;
}
...
{
    if (self.animator.transition == FYCalendarTransitionWeekToMonth) {
...
{
    if (self.animator.transition == FYCalendarTransitionWeekToMonth && self.animator.state == FYCalendarTransitionStateInProgress) {
        return 42;
    }
    if (!self.floatingMode) {
        switch (_scope) {
...
{
    FYCalendarCell *cell = [collectionView dequeueReusableCellWithReuseIdentifier:@"cell" forIndexPath:indexPath];
...
- (UICollectionReusableView *)collectionView:(UICollectionView *)collectionView viewForSupplementaryElementOfKind:(NSString *)kind atIndexPath:(NSIndexPath *)indexPath
{
    if (self.floatingMode) {
        if ([kind isEqualToString:UICollectionElementKindSectionHeader]) {
            FYCalendarStickyHeader *stickyHeader = [collectionView dequeueReusableSupplementaryViewOfKind:UICollectionElementKindSectionHeader withReuseIdentifier:@"header" forIndexPath:indexPath];
...
- (BOOL)collectionView:(UICollectionView *)collectionView shouldSelectItemAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (cell.dateIsPlaceholder) {
        ...
        } else if (![self isDate:cell.date equalToDate:_currentPage toCalendarUnit:FYCalendarUnitMonth]){
...
{
    if ([elementKind isEqualToString:UICollectionElementKindSectionHeader] && [view isKindOfClass:[FYCalendarStickyHeader class]]) {
        [view setNeedsLayout];
    }
}
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView
{
    ...
    if (self.floatingMode && _collectionView.indexPathsForVisibleItems.count) {
        ...
        if (_collectionView.contentOffset.y < 0 || _collectionView.contentOffset.y > _collectionView.contentSize.height-_collectionView.fy_height) {
            return;
        }
        NSDate *currentPage = _currentPage;
        CGPoint significantPoint = CGPointMake(_collectionView.fy_width*0.5,MIN(self.preferredRowHeight*2.75, _collectionView.fy_height*0.5)+_collectionView.contentOffset.y);
        ...
        if (significantIndexPath) {
            ...
            __block FYCalendarStickyHeader *significantHeader = nil;
            [_stickyHeaderMapTable.dictionaryRepresentation.allValues enumerateObjectsUsingBlock:^(FYCalendarStickyHeader *header, NSUInteger idx, BOOL *stop) {
        ...
        if (![self isDate:currentPage equalToDate:_currentPage toCalendarUnit:FYCalendarUnitMonth]) {
        ...
        switch (_collectionViewLayout.scrollDirection) {
            case UICollectionViewScrollDirectionHorizontal: {
                scrollOffset = scrollView.contentOffset.x/scrollView.fy_width;
                break;
            }
            case UICollectionViewScrollDirectionVertical: {
                scrollOffset = scrollView.contentOffset.y/scrollView.fy_height;
                break;
            }
        }
...
- (void)scrollViewWillEndDragging:(UIScrollView *)scrollView withVelocity:(CGPoint)velocity targetContentOffset:(inout CGPoint *)targetContentOffset
{
    ...
    switch (_collectionViewLayout.scrollDirection) {
        case UICollectionViewScrollDirectionHorizontal: {
            ...
            contentSize = scrollView.fy_width;
            break;
        }
        case UICollectionViewScrollDirectionVertical: {
            ...
            contentSize = scrollView.fy_height;
            break;
        }
    }
    ...
    if (shouldTriggerPageChange) {
        ...
        [self currentPageDidChange];
        if (!_showsPlaceholders && self.animator.state == FYCalendarTransitionStateIdle) {
...
- (void)setAppearance:(FYCalendarAppearance *)appearance
{
...
- (FYCalendarAppearance *)appearance
{
    return _appearance;
}

- (void)setScrollDirection:(FYCalendarScrollDirection)scrollDirection
{
...
- (void)setScope:(FYCalendarScope)scope
{
...
- (void)setDataSource:(id<FYCalendarDataSource>)dataSource
{
...
- (void)setOrientation:(FYCalendarOrientation)orientation
{
...
{
    if (_headerHeight == FYCalendarAutomaticDimension) {
        if (_preferredWeekdayHeight == FYCalendarAutomaticDimension) {
            if (!self.floatingMode) {
                CGFloat divider = _scope == FYCalendarScopeMonth ? FYCalendarStandardMonthlyPageHeight : FYCalendarStandardWeeklyPageHeight;
                _preferredHeaderHeight = (FYCalendarStandardHeaderHeight/divider)*self.fy_height;
                _preferredHeaderHeight -= (_preferredHeaderHeight-FYCalendarStandardHeaderHeight)*0.5;
            } else {
                _preferredHeaderHeight = FYCalendarStandardHeaderHeight*MAX(1, FYCalendarDeviceIsIPad*1.5);
            }
        }
...
{
    if (_weekdayHeight == FYCalendarAutomaticDimension) {
        if (_preferredWeekdayHeight == FYCalendarAutomaticDimension) {
            if (!self.floatingMode) {
                CGFloat divider = _scope == FYCalendarScopeMonth ? FYCalendarStandardMonthlyPageHeight : FYCalendarStandardWeeklyPageHeight;
                _preferredWeekdayHeight = (FYCalendarStandardWeekdayHeight/divider)*self.fy_height;
            } else {
                _preferredWeekdayHeight = FYCalendarStandardWeekdayHeight*MAX(1, FYCalendarDeviceIsIPad*1.5);
            }
        }
...
{
    if (_preferredRowHeight == FYCalendarAutomaticDimension) {
        CGFloat headerHeight = self.preferredHeaderHeight;
        CGFloat weekdayHeight = self.preferredWeekdayHeight;
        CGFloat contentHeight = self.fy_height-headerHeight-weekdayHeight;
        CGFloat padding = weekdayHeight*0.1;
        ...
        if (!self.floatingMode) {
            switch (_scope) {
                case FYCalendarScopeMonth: {
                    _preferredRowHeight = _showsPlaceholders ? (contentHeight-padding*2)/6.0 : FYCalendarStandardRowHeight;
                    break;
                }
                case FYCalendarScopeWeek: {
                    _preferredRowHeight = _showsPlaceholders ? contentHeight-padding*2 : FYCalendarStandardRowHeight;
                    break;
                }
            }
        } else {
            _preferredRowHeight = FYCalendarStandardRowHeight*MAX(1, FYCalendarDeviceIsIPad*1.5);
        }
    }
    return _preferredRowHeight;
}

- (id<FYCalendarDelegateAppearance>)delegateAppearance
{
    if (_delegate && [_delegate conformsToProtocol:@protocol(FYCalendarDelegateAppearance)]) {
        return (id<FYCalendarDelegateAppearance>)_delegate;
    }
    return nil;
}
...
- (void)reloadData
{
    ...
    if (![self isDate:minimumDate equalToDate:_minimumDate toCalendarUnit:FYCalendarUnitMonth] || ![self isDate:maximumDate equalToDate:_maximumDate toCalendarUnit:FYCalendarUnitMonth]) {
...
[self didChangeValueForKey:@"scope"]; \
        ...
        FYCalendarScope prevScope = _scope;
        
        if (!self.hasValidateVisibleLayout && prevScope == FYCalendarScopeMonth && scope == FYCalendarScopeWeek) {
            m_set_scope
        ...
        if (self.animator.state == FYCalendarTransitionStateIdle) {
            m_set_scope
...
- (void)deselectDate:(NSDate *)date
{
    ...
    if ([_collectionView.indexPathsForSelectedItems containsObject:indexPath]) {
        ...
        FYCalendarCell *cell = (FYCalendarCell *)[_collectionView cellForItemAtIndexPath:indexPath];
        cell.dateIsSelected = NO;
        [cell setNeedsLayout];
...
- (void)selectDate:(NSDate *)date scrollToDate:(BOOL)scrollToDate forPlaceholder:(BOOL)forPlaceholder
{
    ...
    if (forPlaceholder) {
        ...
        [_collectionView selectItemAtIndexPath:targetIndexPath animated:YES scrollPosition:UICollectionViewScrollPositionNone];
        FYCalendarCell *cell = (FYCalendarCell *)[_collectionView cellForItemAtIndexPath:targetIndexPath];
        [cell performSelecting];
...
- (void)scrollToDate:(NSDate *)date animated:(BOOL)animated
{
    ...
    if (!self.floatingMode) {
        ...
        switch (_collectionViewLayout.scrollDirection) {
            case UICollectionViewScrollDirectionVertical: {
                [_collectionView setContentOffset:CGPointMake(0, scrollOffset * _collectionView.fy_height) animated:animated];
                break;
            }
            case UICollectionViewScrollDirectionHorizontal: {
                [_collectionView setContentOffset:CGPointMake(scrollOffset * _collectionView.fy_width, 0) animated:animated];
                break;
            }
        }
        ...
        if (self.hasValidateVisibleLayout) {
            ...
            CGPoint targetOffset = CGPointMake(0, MIN(headerFrame.origin.y,MAX(0,_collectionView.contentSize.height-_collectionView.fy_bottom)));
...
{
    if (self.animator.transition == FYCalendarTransitionWeekToMonth && self.animator.state == FYCalendarTransitionStateInProgress) {
        return [self dateForIndexPath:indexPath scope:FYCalendarScopeMonth];
    }
...
{
    if (self.floatingMode) {
        return ![self isDate:date equalToDate:_currentPage toCalendarUnit:FYCalendarUnitMonth];
    }
    switch (_scope) {
        case FYCalendarScopeMonth:
            return ![self isDate:date equalToDate:_currentPage toCalendarUnit:FYCalendarUnitMonth];
        case FYCalendarScopeWeek:
            return ![self isDate:date equalToDate:_currentPage toCalendarUnit:FYCalendarUnitWeekOfYear];
    }
}
...
- (void)invalidateLayout
{
    if (!self.floatingMode) {
        ...
        if (!_header) {
            ...
            FYCalendarHeader *header = [[FYCalendarHeader alloc] initWithFrame:CGRectZero];
        ...
        if (_scrollEnabled) {
            if (!_deliver) {
                FYCalendarHeaderTouchDeliver *deliver = [[FYCalendarHeaderTouchDeliver alloc] initWithFrame:CGRectZero];
...
- (void)reloadDataForCell:(FYCalendarCell *)cell atIndexPath:(NSIndexPath *)indexPath
{
    ...
    switch (_scope) {
        case FYCalendarScopeMonth: {
            ...
            cell.dateIsPlaceholder = ![self isDate:cell.date equalToDate:month toCalendarUnit:FYCalendarUnitMonth] || ![self isDateInRange:cell.date];
            if (cell.dateIsPlaceholder) {
...
{
    if (!self.floatingMode) {
        [_collectionView.visibleCells enumerateObjectsUsingBlock:^(FYCalendarCell *cell, NSUInteger idx, BOOL *stop) {
            if (cell.dateIsPlaceholder && [self isDate:cell.date equalToDate:date toCalendarUnit:FYCalendarUnitDay] && !cell.dateIsSelected) {
                cell.dateIsSelected = YES;
                [cell setNeedsLayout];
                *stop = YES;
            }
...
{
    if (self.floatingMode) {
        [_collectionView.visibleCells enumerateObjectsUsingBlock:^(FYCalendarCell *cell, NSUInteger index, BOOL *stop) {
...
- (FYCalendarOrientation)currentCalendarOrientation
{
    CGFloat scale = [UIScreen mainScreen].scale;
    CGSize nativeSize = [UIScreen mainScreen].currentMode.size;
    CGSize sizeInPoints = [UIScreen mainScreen].bounds.size;
    FYCalendarOrientation orientation = scale * sizeInPoints.width == nativeSize.width ? FYCalendarOrientationPortrait : FYCalendarOrientationLandscape;
    return orientation;
}
...
- (FYCalendarCellShape)preferredCellShapeForDate:(NSDate *)date
{
    if (self.delegateAppearance && [self.delegateAppearance respondsToSelector:@selector(calendar:appearance:cellShapeForDate:)]) {
        FYCalendarCellShape cellShape = [self.delegateAppearance calendar:self appearance:self.appearance cellShapeForDate:date];
        return cellShape;
    }
#pragma GCC diagnostic push
#pragma GCC diagnostic ignored "-Wdeprecated-declarations"
    else if (self.delegateAppearance && [self.delegateAppearance respondsToSelector:@selector(calendar:appearance:cellStyleForDate:)]) {
        FYCalendarCellShape cellShape = (FYCalendarCellShape)[self.delegateAppearance calendar:self appearance:self.appearance cellStyleForDate:date];
        return cellShape;
    }
#pragma GCC diagnostic pop
    
    return FYCalendarCellShapeCircle;
}
...
@implementation FYCalendar (DateTools)
...
- (NSDate *)dateByIgnoringTimeComponentsOfDate:(NSDate *)date
{
    ...
    components.hour = FYCalendarDefaultHourComponent;
...
{
    return [self isDate:date equalToDate:_today toCalendarUnit:FYCalendarUnitDay];
}
