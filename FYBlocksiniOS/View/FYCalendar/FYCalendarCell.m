...
@property (readonly, nonatomic) FYCalendarCellShape cellShape;
...
- (instancetype)initWithFrame:(CGRect)frame
{
    ...
    if (self) {
        ...
        FYCalendarEventIndicator *eventIndicator;
        ...
        eventIndicator = [[FYCalendarEventIndicator alloc] initWithFrame:CGRectZero];
...
- (void)setBounds:(CGRect)bounds
{
    ...
    diameter = diameter > FYCalendarStandardCellDiameter ? (diameter - (diameter-FYCalendarStandardCellDiameter)*0.5) : diameter;
...
#define kAnimationDuration FYCalendarDefaultBounceAnimationDuration
...
- (void)configureCell
{
    ...
    if (_needsAdjustingViewFrame || CGSizeEqualToSize(_titleLabel.frame.size, CGSizeZero)) {
        ...
        if (_subtitle) {
            ...
            _titleLabel.frame = CGRectMake(0,
                                           (self.contentView.fy_height*5.0/6.0-height)*0.5+_appearance.titleVerticalOffset,
                                           self.fy_width,
                                           titleHeight);
            
            _subtitleLabel.frame = CGRectMake(0,
                                              _titleLabel.fy_bottom - (_titleLabel.fy_height-_titleLabel.font.pointSize)+_appearance.subtitleVerticalOffset,
                                              self.fy_width,
                                              subtitleHeight);
        } else {
            _titleLabel.frame = CGRectMake(0, _appearance.titleVerticalOffset, self.contentView.fy_width, floor(self.contentView.fy_height*5.0/6.0));
        }
    ...
    if (!shouldHideShapeLayer) {
        ...
        CGPathRef path = self.cellShape == FYCalendarCellShapeCircle ?
...
- (UIColor *)colorForCurrentStateInDictionary:(NSDictionary *)dictionary
{
    if (self.isSelected || self.dateIsSelected) {
        if (self.dateIsToday) {
            return dictionary[@(FYCalendarCellStateSelected|FYCalendarCellStateToday)] ?: dictionary[@(FYCalendarCellStateSelected)];
        }
        return dictionary[@(FYCalendarCellStateSelected)];
    }
    if (self.dateIsToday && [[dictionary allKeys] containsObject:@(FYCalendarCellStateToday)]) {
        return dictionary[@(FYCalendarCellStateToday)];
    }
    if (self.dateIsPlaceholder && [[dictionary allKeys] containsObject:@(FYCalendarCellStatePlaceholder)]) {
        return dictionary[@(FYCalendarCellStatePlaceholder)];
    }
    if (self.isWeekend && [[dictionary allKeys] containsObject:@(FYCalendarCellStateWeekend)]) {
        return dictionary[@(FYCalendarCellStateWeekend)];
    }
    return dictionary[@(FYCalendarCellStateNormal)];
}
