...
@implementation FYCalendarAnimator
...
- (void)performScopeTransitionFromScope:(FYCalendarScope)fromScope toScope:(FYCalendarScope)toScope animated:(BOOL)animated
{
    if (fromScope == toScope) {
        self.transition = FYCalendarTransitionNone;
        return;
    }
    if (fromScope == FYCalendarScopeMonth && toScope == FYCalendarScopeWeek) {
        self.transition = FYCalendarTransitionMonthToWeek;
    } else if (fromScope == FYCalendarScopeWeek && toScope == FYCalendarScopeMonth) {
        self.transition = FYCalendarTransitionWeekToMonth;
    }
    
    // Start transition
    self.state = FYCalendarTransitionStateInProgress;
    
    switch (self.transition) {
            
        case FYCalendarTransitionMonthToWeek: {
            
            CGSize contentSize = [self.calendar sizeThatFits:self.calendar.frame.size scope:FYCalendarScopeWeek];
            ...
            if (self.calendar.focusOnSingleSelectedDate) {
                ...
                if (focusedDate) {
                    UICollectionViewLayoutAttributes *attributes = [self.collectionViewLayout layoutAttributesForItemAtIndexPath:[self.calendar indexPathForDate:focusedDate scope:FYCalendarScopeMonth]];
            ...
            NSDate *firstDate = [self.calendar dateForIndexPath:firstIndexPath scope:FYCalendarScopeMonth];
            currentPage = [self.calendar dateByAddingDays:focusedRowNumber*7 toDate:firstDate];
            
            Ivar currentPageIvar = class_getInstanceVariable(FYCalendar.class, "_currentPage");
            ...
            if (animated) {
                ...
                opacity.toValue = @0;
                [self.collectionView.visibleCells enumerateObjectsUsingBlock:^(FYCalendarCell *cell, NSUInteger idx, BOOL *stop) {
                ...
                [CATransaction setCompletionBlock:^{
                    self.state = FYCalendarTransitionStateIdle;
                    self.transition = FYCalendarTransitionNone;
                ...
                if (self.calendar.delegate && ([self.calendar.delegate respondsToSelector:@selector(calendar:boundingRectWillChange:animated:)] || [self.calendar.delegate respondsToSelector:@selector(calendarCurrentScopeWillChange:animated:)])) {
                    ...
                    self.collectionView.fy_top = -focusedRowNumber*self.calendar.preferredRowHeight;
                    self.calendar.bottomBorder.fy_top = CGRectGetMaxY(targetBounds);
...
#pragma GCC diagnostic pop
                ...
                self.calendar.header.scrollDirection = self.collectionViewLayout.scrollDirection;
                self.calendar.needsAdjustingViewFrame = YES;
                self.calendar.bottomBorder.frame = CGRectMake(0, contentSize.height, self.calendar.fy_width, 1);
                self.calendar.maskLayer.path = [UIBezierPath bezierPathWithRect:targetBounds].CGPath;
                self.calendar.bottomBorder.fy_top = CGRectGetMaxY(targetBounds);
                [self.collectionView reloadData];
                [self.collectionView layoutIfNeeded];
...
#pragma GCC diagnostic pop
        ...
        case FYCalendarTransitionWeekToMonth: {
            ...
            CGSize contentSize = [self.calendar sizeThatFits:self.calendar.frame.size scope:FYCalendarScopeMonth];
            CGRect targetBounds = (CGRect){CGPointZero,contentSize};
            ...
            if (self.calendar.focusOnSingleSelectedDate) {
                NSDate *focusedDate = self.calendar.selectedDate;
                if (focusedDate) {
                    UICollectionViewLayoutAttributes *attributes = [self.collectionViewLayout layoutAttributesForItemAtIndexPath:[self.calendar indexPathForDate:focusedDate scope:FYCalendarScopeWeek]];
                    CGPoint focuedCenter = attributes.center;
                    if (CGRectContainsPoint(self.collectionView.bounds, focuedCenter)) {
            ...
            for (int i = 0; i < 6; i++) {
                ...
                if ([self.calendar isDate:currentRow equalToDate:currentPage toCalendarUnit:FYCalendarUnitDay]) {
                    focusedRowNumber = i;
            ...
            if (animated) {
                ...
                if (self.calendar.delegate && ([self.calendar.delegate respondsToSelector:@selector(calendar:boundingRectWillChange:animated:)] || [self.calendar.delegate respondsToSelector:@selector(calendarCurrentScopeWillChange:animated:)])) {
                    ...
                    [UIView setAnimationCurve:UIViewAnimationCurveEaseInOut];
                    [UIView setAnimationDuration:duration];
                    self.collectionView.fy_top = 0;
                    self.self.calendar.bottomBorder.frame = CGRectMake(0, contentSize.height, self.calendar.fy_width, 1);
                    if ([self.calendar.delegate respondsToSelector:@selector(calendar:boundingRectWillChange:animated:)]) {
                        [self.calendar.delegate calendar:self.calendar boundingRectWillChange:targetBounds animated:animated];
                    } else {
#pragma GCC diagnostic push
...
- (void)performBoudingRectTransitionFromMonth:(NSDate *)fromMonth toMonth:(NSDate *)toMonth duration:(CGFloat)duration
{
    ...
    if (lastRowCount != currentRowCount) {
        ...
        [UIView animateWithDuration:animationDuration delay:0  options:UIViewAnimationOptionAllowUserInteraction animations:^{
            ...
            self.calendar.bottomBorder.fy_top = CGRectGetMaxY(bounds);
