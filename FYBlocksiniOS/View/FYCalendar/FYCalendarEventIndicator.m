...
@interface FYCalendarEventIndicator ()
...
@implementation FYCalendarEventIndicator
...
- (instancetype)initWithFrame:(CGRect)frame
{
    ...
    if (self) {
        ...
        for (int i = 0; i < 3; i++) {
            ...
            layer.backgroundColor = FYCalendarStandardEventDotColor.CGColor;
...
- (void)layoutSubviews
{
    ...
    if (_needsAdjustingViewFrame) {
        CGFloat diameter = MIN(MIN(self.fy_width, self.fy_height),FYCalendarMaximumEventDotDiameter);
        self.contentView.fy_height = self.fy_height;
        self.contentView.fy_width = (self.numberOfEvents*2-1)*diameter;
    ...
    if (layer == self.layer) {
        if (_needsAdjustingViewFrame) {
            ...
            for (int i = 0; i < self.eventLayers.count; i++) {
                ...
                if (!layer.hidden) {
                    layer.frame = CGRectMake(2*i*diameter, (self.fy_height-diameter)*0.5, diameter, diameter);
